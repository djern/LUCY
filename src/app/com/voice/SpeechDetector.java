...
public class SpeechDetector {
	...
	public boolean checkIfSpeaking(byte[] targetData){
		...
		if(dBSPL > Config.SPEECH_dBSPL_THRESHOLD){
			momentDetection = System.currentTimeMillis();
			if(speaking == false) {
				firstDetection = momentDetection;
				lastDetection = momentDetection;
			}
			if(((momentDetection - lastDetection)/1000 <= Config.SPEECH_SENTENCE_PAUSE) && speaking == false) {
				speaking = true;
			}
			else if(((momentDetection - lastDetection)/1000 <= Config.SPEECH_SENTENCE_PAUSE) && speaking == true) {
				lastDetection = momentDetection;
//				System.out.println("Sound detected for:" + (double)(momentDetection - firstDetection)/1000 + " seconds, " + (int)dBSPL + "dB SPL\n");
		...
		else if(dBSPL <= Config.SPEECH_dBSPL_THRESHOLD && (System.currentTimeMillis() - lastDetection)/1000 <= Config.SPEECH_SENTENCE_PAUSE)
			speaking = true;
		else
			speaking = false;
		return speaking;
